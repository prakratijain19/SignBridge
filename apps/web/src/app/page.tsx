...
export default async function HomePage() {
  ...
  const dbConnected = health?.success === true && health.data.dependencies.database === 'connected';
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 max-w-prose text-lg text-ink/70">
        Bridging communication beyond language and hearing barriers. This is the project scaffold —
        the full experience is built up phase by phase.
      </p>

      <section aria-labelledby="status-heading" className="mt-12">
        <h2
          id="status-heading"
          className="text-sm font-semibold uppercase tracking-wider text-ink/50"
        >
          System status
        </h2>
        <dl className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <StatusCard label="Database" ok={dbConnected} okText="Connected" badText="Disconnected" />
          <StatusCard label="Shared types" ok={true} okText="Linked" badText="—" />
        </dl>
        {!online && (
          <p className="mt-4 text-sm text-ink/60">
            The API is not responding. Start it with{' '}
            <code className="rounded bg-ink/5 px-1.5 py-0.5">pnpm dev</code> and make sure
            PostgreSQL is running (
            <code className="rounded bg-ink/5 px-1.5 py-0.5">pnpm docker:up</code>).
          </p>
        )}
      </section>
    </main>
